...
(defun wiki-summary/extract-summary (resp)
  "Given the JSON response from the webpage, grab the summary as a string"
  (let* ((query (plist-get resp 'query))
         (pages (plist-get query 'pages))
         current-page result)
    (with-temp-buffer
      (setq-local fill-column 60)
      (dolist (page-id pages)
        (setq current-page (cadr pages))
        (when (and current-page (plist-get current-page 'extract))
          (setq pages (cddr pages))
          (insert (plist-get current-page 'extract))))
