...
(defcustom hledger-currencies (list "₹" "€")
  "List of currency strings that the user is prompted to choose from."
  :group 'hledger
  :type 'list)
...
(defvar hledger-account-regex "\\(\\([Rr]evenues\\|[aA]ssets\\|[lL]iabilities\\|[Ee]quity\\|[Ee]xpenses\\|[iI]ncome\\|[Zz]adjustments\\)\\(:[0-9A-Za-z--]+\\)+\\)"
...
(defvar hledger-amount-value-regex "[-]?[0-9][0-9,]*\\(\\.[0-9]+\\)?"
  "Regular expression to match a floating point number.")
(defvar hledger-amount-regex
  (format "[%s]\\s-*[-]?[0-9][0-9,]*\\(\\.[0-9]+\\)?"
          (mapconcat 'identity hledger-currencies ""))
