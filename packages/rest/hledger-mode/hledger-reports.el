...
(defcustom hledger-show-expanded-report nil
...
Optional argument KEEP-BUFFERP if non-nil the
`hledger-reporting-buffer-name' is re-used without erasing its
contents.
    ...
    (pcase command
      ...
      (`"cashflow"
       (hledger-jdo (format "cashflow --begin %s --end %s"
                            (hledger-format-time (hledger-beg-reporting-time))
                            (hledger-format-time (hledger-end-reporting-time)))))
...
(defun hledger-jdo (command &optional keep-bufferp bury-bufferp extra-args)
