...
If ARG is supplied, reset margins and fringes to zero."
  ...
  (let* ((max-text-width (save-excursion
         ...
         (margin-width (if arg
                           0
                         (/ (max (- (window-text-width) max-text-width) 0) 2))))
...
(defvar quick-switch-themes (let ((themes-list (list 'jazz 'modus-operandi 'ef-dark 'ef-dream)))
  ...
  (let ((frame (selected-frame)))
    (if-let* ((params (get this-command :original-parameters)))
      ...
      (let ((original-params (frame-parameters))
            (height 30)
            (width 96)
            (alpha-background (if prefix 0.4 1.0))
            (x 326)
            (y 40))
...
(defun augment-srt-file (input-srt output-srt anki-output-tsv language proficiency-level)
  ...
  (interactive
   (list (read-file-name "Input SRT file: ")
         ...
         (read-file-name "Anki Output TSV:" )
  ...
  (let* ((api-key (gptel-api-key-from-auth-source "api.groq.com"))
    ...
    (compile (format "%s %s %s %s %s %s"
                     ...
                     (shell-quote-argument (expand-file-name anki-output-tsv))
