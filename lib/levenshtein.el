...
;;;###autoload
(defun levenshtein-sort (name candidates)
  "Return CANDIDATES sorted by similarity with NAME.
Since this is terribly slow, do not sort if CANDIDATES has more
than 20 members."
  ;; This performs terribly even for 10 candidates.
  (if (< (length candidates) 20)
      (seq-sort-by (apply-partially #'levenshtein-distance name)
                   #'<
                   candidates)
    candidates))
